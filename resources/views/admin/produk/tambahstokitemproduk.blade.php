...
<div class="card">
    <div class="card-body">
        ...
        <div class="row" style="margin-left: 20px; margin-right: 20px">
            <p class="text-center">Ukuran : <b>{{$item_produk->ukuran->nama_ukuran}}</b> | Warna:
                <b>{{$item_produk->warna->nama_warna}}</b> | Stok Lama : <b
                    class=" badge text-center @if($item_produk->jumlah_stok <= 10) bg-danger @elseif($item_produk->jumlah_stok > 10 && $item_produk->jumlah_stok <= 30) bg-warning @elseif($item_produk->jumlah_stok > 30) bg-success @endif">{{ $item_produk->jumlah_stok }}</b>
            </p>
            <form action="{{ route('insertdatastokitemproduk', ['id' => $item_produk->id]) }}" method="POST"
